...
namespace internal{
  ...
  struct SharedState{
    ...
    State WaitFor(const std::chrono::duration<Rep, Period>& timeout_duration) const {
      std::unique_lock<std::mutex> lock(then_mtx_);
    ...
    State WaitUntil(const std::chrono::time_point<Clock, Duration>& timeout_time) const {
      std::unique_lock<std::mutex> lock(then_mtx_);
  ...
  public:
    ...
    void SetException(std::exception_ptr&& exp){
      SetValue(std::move(exp));
    }

    bool IsReady(){
      return shared_state_->state_ != internal::State::None;
    }
  ...
  private:
    ...
    template<typename V>
    void SetValueInternal(V&& val){
      shared_state_->value_ = absl::forward<V>(val);
    }

    void SetValueInternal(){
      shared_state_->value_ = Try<void>();
    }

    void SetValueInternal(std::exception_ptr&& e){
      shared_state_->value_ = typename internal::SharedState<T>::ValueType(std::move(e));
    }
  private:
  ...
  absl::enable_if_t<std::is_void<R>::value> SetValue(P& promise, F&& fn, Tuple&& tp){
  ...
  Future<typename function_traits<F>::return_type> Async(F&& fn, Args&&... args) {
    return AsyncImpl(Lauch::Async, (EmpEx*)nullptr, std::forward<F>(fn), std::forward<Args>(args)...);
  }

  template<typename Executor, typename F, typename... Args>
  Future<typename function_traits<F>::return_type> Async(Executor* ex, F&& fn, Args&&... args) {
    return AsyncImpl(Lauch::Pool, ex, std::forward<F>(fn), std::forward<Args>(args)...);
  }

  template<typename Executor, typename F, typename... Args>
  Future<typename function_traits<F>::return_type> AsyncImpl(Lauch policy, Executor* ex, F&& fn, Args&&... args){
  ...
  public:
    ...
    explicit Future(std::shared_ptr<internal::SharedState<T>> state) : shared_state_(std::move(state)){}
    ...
    Future<typename function_traits<F>::return_type> Then(Executor& executor, F&& fn){
      return ThenImpl(Lauch::Pool, &executor, std::forward<F>(fn));
    }

    template<typename F, typename Executor>
    Future<typename function_traits<F>::return_type> ThenImpl(Lauch policy, Executor*executor, F&& fn){
      static_assert(function_traits<F>::arity <= 1, "Then must take zero or one argument");
      ...
      std::unique_lock<std::mutex> lock(shared_state_->then_mtx_);
      if(shared_state_->state_==internal::State::None){
        shared_state_->then_ = [policy, executor, func, next_prom, this](typename TryWrapper<T>::type&& t) mutable {
...
//        auto arg = MakeMoveWrapper(std::move(t));
//        Invoke<first>(std::move(func), std::move(arg), std::move(next_prom));
//        auto result = Invoke<first>(func.move(), std::move(t));
//        next_prom.move().SetValue(std::move(result));
    ...
    template < typename Rep, typename Period >
    internal::State Wait_for(const std::chrono::duration<Rep, Period>& timeout_duration) const {
      return shared_state_->wait_for(timeout_duration);
    }

    template < typename Clock, typename Duration >
    internal::State WaitUntil(const std::chrono::time_point<Clock, Duration>& timeout_time) const{
...
//    }
  ...
  private:
    ...
    auto Invoke(F fn, Arg arg) ->
      typename std::enable_if<!IsTry<U>::value&&std::is_same<void, U>::value, try_type_t<decltype(fn())>>::type {
    ...
    template<typename U, typename F, typename Arg>
    auto Invoke(F fn, Arg arg) ->
      typename std::enable_if<!IsTry<U>::value&&!std::is_same<void, U>::value, try_type_t<decltype(fn(arg.Value()))>>::type {
    ...
    template<typename U, typename F, typename Arg>
    auto Invoke(F fn, Arg arg) ->
      typename std::enable_if<IsTry<U>::value&&!std::is_same<void, typename IsTry<U>::Inner>::value,
    ...
    template<typename U, typename F, typename Arg>
    auto Invoke(F fn, Arg arg) ->
    typename std::enable_if<IsFuture<U>::value&&!std::is_same<void, typename IsFuture<U>::Inner>::value,
        try_type_t<decltype(fn(std::move(arg)))>>::type {
      using type = decltype(fn(std::move(arg)));
      return try_type_t<type>(fn(std::move(arg)));
    }
